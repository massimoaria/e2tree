...
# ===========================================================================
# BACKWARD COMPATIBILITY: keep goi(), goi_perm(), goi_analysis() as aliases
# ===========================================================================
...
goi_perm <- function(O, O_hat, n_perm = 999,
  ...
  if (graph) {
    ...
    plot(density(z), type = "l",
         main = "Standardized Null Distribution of LoI",
         xlab = "Z-score", ylab = "Density", lwd = 2)
  ...
  result <- list(
    statistic = G_obs, p.value = p_value, ci = ci,
    conf.level = conf.level, alternative = alternative,
    n_perm = n_perm, null_dist = perm_G,
    null_mean = null_mean, null_sd = null_sd,
    z_stat = z_stat, n_na = n_na
  )
...
print.goi_perm <- function(x, digits = 4, ...) {
  ...
  cat("   Permutation Test for Loss of Interpretability (LoI)\n")
  cat("==============================================================================\n\n")
  cat(sprintf("  Observed LoI:        %.*f\n", digits, x$statistic))
  cat(sprintf("  Null mean:           %.*f\n", digits, x$null_mean))
  cat(sprintf("  Null SD:             %.*f\n", digits, x$null_sd))
  cat(sprintf("  Z-statistic:         %.*f\n", digits, x$z_stat))
  cat("\n------------------------------------------------------------------------------\n")
  alt_text <- switch(x$alternative,
                     "greater" = "LoI > expected by chance",
                     "less" = "LoI < expected by chance",
                     "two.sided" = "LoI != expected by chance")
  cat(sprintf("  H1:                  %s\n", alt_text))
  if (x$p.value < 0.0001) {
    cat("  p-value:             < 0.0001 ***\n")
  } else {
    stars <- if (x$p.value < 0.001) "***" else if (x$p.value < 0.01) "**"
             else if (x$p.value < 0.05) "*" else if (x$p.value < 0.1) "." else ""
    cat(sprintf("  p-value:             %.*f %s\n", digits, x$p.value, stars))
  }
  cat(sprintf("  %d%% CI:             [%.*f, %.*f]\n",
              round(x$conf.level * 100), digits, x$ci[1], digits, x$ci[2]))
...
plot.goi_perm <- function(x, ...) {
  ...
  hist(x$null_dist,
       main = "Null Distribution of LoI (Permutation Test)",
       xlab = "LoI", col = "lightgray", border = "white",
  ...
  if (x$p.value < 0.0001) mtext("p < 0.0001", side = 3, line = -2, adj = 0.95)
  else mtext(sprintf("p = %.4f", x$p.value), side = 3, line = -2, adj = 0.95)
}
...
print.goi_analysis <- function(x, digits = 4, ...) {
  ...
  cat(sprintf("  %d%% CI:             [%.*f, %.*f]\n",
              ...
              digits, x$ci[1], digits, x$ci[2]))
  cat("\n------------------------------------------------------------------------------\n")
  cat("  Permutation Test (H1: LoI < chance)\n")
  cat("------------------------------------------------------------------------------\n")
  if (x$p.value < 0.0001) {
    cat("  p-value:             < 0.0001 ***\n")
  } else {
